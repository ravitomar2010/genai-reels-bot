...
Both the on-screen card text (hook, title, points, CTA, thumbnail) and the spoken narration are Hinglish — casual Hindi-English code-switching, written entirely in Roman/Latin script (e.g. "Zyada log AI ka istemal galat tarike se karte hain"), never Devanagari. The same `hook`/`title`/`points`/`cta` fields Claude generates drive both the rendered cards and the `hi-IN-SwaraNeural` edge-tts narration — one field, no separate translation step.

Devanagari was tried first but dropped: Pillow doesn't do correct complex-script shaping without extra system libraries this environment doesn't have, so pre-base vowel signs (matras) rendered in the wrong position — e.g. "मिलता" came out as "मलिता". Roman script sidesteps the issue entirely since there's no glyph reordering involved. `strip_emoji()`'s character whitelist deliberately excludes Devanagari so any stray character Claude outputs gets silently dropped rather than rendered corrupted.

YouTube's `title`/`description`/`tags` and the Instagram caption stay in English deliberately, for search discoverability — only the video's own text and audio are Hinglish.

The static topic rotation (`topics_genai.json`) is still English-only, so a Claude API failure falls back to narrating/rendering English text through the Hindi voice for that one run.
